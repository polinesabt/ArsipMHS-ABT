...
function AdminLayoutShell() {
  ...
  const activeItem = useMemo((): AdminNavItem => {
    ...
    if (pathname.startsWith('/admin/mahasiswa/kustom-form') || pathname.startsWith('/admin/mahasiswa/evaluasi')) {
      const evaluasi = ADMIN_NAV_ITEMS.find((item) => item.id === 'evaluasi-lulusan');
      return evaluasi ?? ADMIN_NAV_ITEMS[0];
    }
    if (pathname.startsWith('/admin/mahasiswa/history-logbook')) {
      const item = ADMIN_NAV_ITEMS.find((i) => (i as { path?: string }).path === '/admin/mahasiswa/history-logbook');
  ...
  return (
    <div className="min-h-screen bg-background overflow-x-hidden">
      ...
      >
        <div className="h-14 sm:h-16 border-b border-border/50 bg-card/80 backdrop-blur-xl shadow-sm px-3 sm:px-6 flex items-center justify-between">
