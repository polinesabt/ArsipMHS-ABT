...
import {
  ...
  type PKMItem
} from '@/data/mockPengabdianDosenData';

export default function AdminDosenPengabdianPage() {
  const { toast } = useToast();
  const { kontribusiPengabdianList: dosenList, updateKontribusiPengabdian } = useDosen();
  const [searchTerm, setSearchTerm] = useState('');
  const [expandedNidns, setExpandedNidns] = useState<Set<string>>(new Set());

  // State: Slide-over Edit Sidebar
  const [isEditSheetOpen, setIsEditSheetOpen] = useState(false);
  const [editingDosen, setEditingDosen] = useState<KontribusiPengabdianDosenItem | null>(null);
  const [formData, setFormData] = useState<KontribusiPengabdianDosenItem | null>(null);
  ...
  const openEditModal = (dosen: KontribusiPengabdianDosenItem) => {
  ...
  // Form Handlers: Kegiatan PKM & Kerjasama Instansi
  const handleAddPKM = () => {
    if (!formData) return;
    const newId = `pkm-custom-${Date.now()}`;
    const newPKM: PKMItem = {
      id: newId,
      namaKegiatan: '',
      kerjasamaInstansi: '',
    ...
    setFormData({
      ...formData,
      pkm: [...formData.pkm, newPKM],
    });

    // Auto-scroll to the newly created PKM box & focus the first input (Nama Kegiatan PKM)
    setTimeout(() => {
  ...
  const handleUpdatePKM = (
    ...
    setFormData({
      ...formData,
      pkm: formData.pkm.map((p) =>
        p.id === id ? { ...p, [field]: val } : p
      ),
    });
  };

  const handleDeletePKM = (id: string) => {
    if (!formData) return;
    setFormData({
      ...formData,
      pkm: formData.pkm.filter((p) => p.id !== id),
    });
  };
  ...
  const handleAddRekognisi = () => {
    ...
    setTimeout(() => {
      const input = document.getElementById(`rekognisi-pkm-input-${newIndex}`) as HTMLInputElement | null;
      if (input) {
  ...
  const handleSave = () => {
    ...
    // Clean up blank rows
    const cleanedPKM = formData.pkm.filter((p) => p.namaKegiatan.trim() !== '');
    const cleanedRekognisi = formData.rekognisi.filter((r) => r.trim() !== '');

    const finalData: KontribusiPengabdianDosenItem = {
      ...formData,
      pkm: cleanedPKM,
    ...
    toast({
      ...
      description: `Kontribusi pengabdian masyarakat untuk ${finalData.nama} telah tersimpan.`,
    });
  };
  ...
  const filteredData = useMemo(() => {
    ...
    return dosenList.filter((dosen) => {
      ...
      // Check PKM Nama Kegiatan, Instansi Mitra, Skema
      const matchPKM = dosen.pkm.some(
        (p) =>
  ...
  return (
    <div className="space-y-6 pb-14 font-sans selection:bg-primary/20 selection:text-primary">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-lg font-bold text-foreground">Kontribusi Pengabdian Dosen ABT</h2>
              <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-primary/10 text-primary border border-primary/20">
                {filteredData.length} Dosen
              </span>
            </div>
            <p className="text-xs text-muted-foreground mt-0.5">
              Rincian kegiatan Pengabdian Kepada Masyarakat (PKM), kerjasama dengan instansi/komunitas mitra, serta rekognisi dan kepakaran dosen Administrasi Bisnis Terapan.
            </p>
          </div>
          ...
          <div className="relative w-full sm:w-80">
            ...
            <Input
              placeholder="Cari nama, NIDN/NIDK, nama PKM, mitra, rekognisi..."
      ...
      {/* 3. INTERACTIVE COLLAPSIBLE CARD LIST */}
      <div className="space-y-3.5">
        {filteredData.length > 0 ? (
          filteredData.map((dosen, index) => {
            ...
            // Preview chips: first 2 PKM activities when collapsed for symmetrical 1-line preview
            const previewChips = dosen.pkm.slice(0, 2);
            const remainingCount = dosen.pkm.length - 2;
                  ...
                  {isExpanded && (
                    ...
                    >
                      <div className="p-4 sm:p-6 space-y-6">
                        {/* BAGIAN 1: TABEL KEGIATAN PKM & KERJASAMA INSTANSI */}
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <h4 className="text-xs sm:text-sm font-bold text-foreground">
                                  1. Nama Kegiatan PKM & Kerjasama Instansi/Organisasi
                                </h4>
                                <p className="text-[11px] text-muted-foreground">
                                  Rekam jejak program pengabdian masyarakat, pelatihan, pendampingan UMKM, dan kemitraan komunitas
                                </p>
                              </div>
                            </div>
                              ...
                              <table className="w-full text-xs text-left">
                                <thead className="bg-muted/60 text-muted-foreground font-semibold border-b border-border/70">
                                  <tr>
                                    ...
                                    <th className="px-3.5 py-2.5 min-w-[280px]">Nama Kegiatan PKM</th>
                                    <th className="px-3.5 py-2.5 min-w-[220px]">Kerjasama Instansi / Komunitas Mitra</th>
                                    <th className="px-3.5 py-2.5 w-32 text-center">Tahun / Skema</th>
                                  </tr>
                                </thead>
                                <tbody className="divide-y divide-border/50 font-normal">
                                  {dosen.pkm.length > 0 ? (
                                    dosen.pkm.map((p, pIdx) => {
                                    ...
                                    <tr>
                                      <td colSpan={4} className="px-4 py-6 text-center text-muted-foreground">
                                        Belum ada data kegiatan PKM yang dicatat untuk dosen ini.
                                      </td>
                                    </tr>
                                  )}
                                </tbody>
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                ...
                                <p className="text-[11px] text-muted-foreground">
                                  Pengakuan kepakaran, narasumber pelatihan, instruktur, konsultan pendamping, juri atau pembicara
                                </p>
                              </div>
                            </div>
          ...
          <div className="glass-card rounded-2xl p-12 text-center border border-border/70 bg-card shadow-soft space-y-3">
            ...
            <p className="text-xs text-muted-foreground max-w-md mx-auto">
              Tidak ditemukan data pengabdian dosen dengan kata kunci &quot;<strong className="text-foreground">{searchTerm}</strong>&quot;. Silakan periksa kembali ejaan pencarian Anda.
            </p>
            <Button
              variant="outline"
              size="sm"
      ...
      <Sheet open={isEditSheetOpen} onOpenChange={handleSheetOpenChange}>
        ...
        >
            ...
            <SheetHeader className="text-left space-y-1">
              <div className="flex items-center gap-2">
                <SheetTitle className="text-lg font-bold text-foreground">
                  Edit Kontribusi Pengabdian Dosen
                </SheetTitle>
                {isDirty && (
              ...
              <SheetDescription className="text-xs text-muted-foreground">
                Perbarui daftar nama kegiatan PKM, instansi/komunitas mitra, dan rekognisi keahlian untuk dosen ini.
              </SheetDescription>
            </SheetHeader>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {formData && (
              <>
                {/* SECTION A: KELOLA KEGIATAN PKM & KERJASAMA INSTANSI */}
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <HeartHandshake className="w-4 h-4 text-primary" />
                      <h4 className="text-xs font-bold uppercase tracking-wider text-foreground">
                        Daftar Nama Kegiatan PKM & Kerjasama Mitra
                      </h4>
                    </div>
                    <Button
                      type="button"
                  ...
                  <div className="space-y-3">
                    {formData.pkm.map((p, idx) => {
                      const isNew = p.id.startsWith('pkm-custom-');
                        ...
                        >
                          ...
                          <div className="space-y-1">
                            <label className="text-[11px] font-semibold text-foreground flex items-center justify-between">
                              <span>Nama Kegiatan PKM</span>
                              {isNew && (
                                <span className="text-[10px] text-primary font-medium">
                                  (Ketik nama kegiatan di sini)
                                </span>
                              )}
                            </label>
                            <Input
                              placeholder="Masukkan nama kegiatan PKM lengkap..."
                              value={p.namaKegiatan}
                              onChange={(e) => handleUpdatePKM(p.id, 'namaKegiatan', e.target.value)}
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                            <div className="space-y-1">
                              <label className="text-[11px] font-semibold text-foreground flex items-center gap-1">
                                ...
                                <span>Kerjasama Instansi / Komunitas Mitra</span>
                              </label>
                              <Input
                                placeholder="Nama mitra (misal: Dinas Koperasi, UMKM Tembalang, Mandiri)..."
                                value={p.kerjasamaInstansi}
                                onChange={(e) => handleUpdatePKM(p.id, 'kerjasamaInstansi', e.target.value)}
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <div className="space-y-1">
                                ...
                                <Input
                                  placeholder="2024"
                                  value={p.tahun || ''}
                                  onChange={(e) => handleUpdatePKM(p.id, 'tahun', e.target.value)}
                              ...
                              <div className="space-y-1">
                                ...
                                <Input
                                  placeholder="Pemberdayaan / Kemitraan"
                                  value={p.skema || ''}
                                  onChange={(e) => handleUpdatePKM(p.id, 'skema', e.target.value)}
                                  className="h-9 text-xs bg-background rounded-lg border-border/70 focus:ring-1 focus:ring-primary"
                                />
                              </div>
                            </div>
                  ...
                  <div className="space-y-2.5">
                    {formData.rekognisi.map((rek, idx) => (
                      <div key={idx} className="flex items-center gap-2">
                        ...
                        <Input
                          id={`rekognisi-pkm-input-${idx}`}
                          placeholder="Nama rekognisi/penghargaan (misal: Narasumber Nasional, Instruktur Pelatihan, Juri)..."
                          value={rek}
        ...
        <AlertDialogContent className="rounded-2xl border-border/70 bg-card font-sans">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-xs text-muted-foreground">
              Ada perubahan data kegiatan PKM atau rekognisi yang belum disimpan. Jika ditutup sekarang, seluruh perubahan akan dibatalkan.
            </AlertDialogDescription>
          </AlertDialogHeader>
