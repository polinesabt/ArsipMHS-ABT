...
$backupPath = 'C:/Users/HP/Downloads/arsw7919_arsipmhs.sql';
if (!file_exists($backupPath)) {
    echo "Backup file not found at: $backupPath\n";
    exit(1);
}

echo "Generating Full Migration + Backup Data (IF NOT EXISTS logic)...\n";
...
$output[] = "-- FULL PRODUCTION MIGRATION & DATA RESTORATION (IF NOT EXISTS LOGIC)";
$output[] = "-- =====================================================================";
$output[] = "-- Total Objek Database: " . count($allObjects) . " (Base Tables: " . count($baseTables) . ", Views: " . count($views) . ")";
$output[] = "-- Karakteristik: Aman untuk Production (Non-Destructive, Idempotent, & Data Restoring)";
$output[] = "-- Jaminan      : 1. Seluruh 71 tabel & view diperbarui ke arsitektur terbaru.";
$output[] = "--                2. Data backup dari 'arsw7919_arsipmhs.sql' disinkronkan";
$output[] = "--                   menggunakan logika IF NOT EXISTS (INSERT IGNORE).";
$output[] = "--                3. Data yang sudah ada di production TIDAK akan tertimpa/rusak.";
$output[] = "-- =====================================================================\n";
...
$columnUpdates = [
    ...
    // record_change_logs
    ["record_change_logs", "deleted_at", "ALTER TABLE record_change_logs ADD COLUMN deleted_at TIMESTAMP NULL AFTER new_data"],
];
...
$output[] = "-- =====================================================================";
$output[] = "-- BAGIAN 3: RESTORASI DATA BACKUP DENGAN LOGIKA IF NOT EXISTS (INSERT IGNORE)";
$output[] = "-- =====================================================================\n";
...
while (($line = fgets($handle)) !== false) {
    // Cari awal statement INSERT INTO
    if (preg_match('/^INSERT INTO\s+`([^`]+)`/i', $line)) {
        $insideInsert = true;
        // Ganti INSERT INTO dengan INSERT IGNORE INTO
        $modifiedLine = preg_replace('/^INSERT INTO/i', 'INSERT IGNORE INTO', $line);
        $currentInsertBuffer = $modifiedLine;
        
        // Cek apakah query selesai di baris yang sama
        if (preg_match('/;\s*$/', trim($line))) {
            $output[] = $currentInsertBuffer;
            $insideInsert = false;
            $currentInsertBuffer = '';
            $insertCount++;
        }
...
echo "Extracted and converted $insertCount INSERT statements to INSERT IGNORE INTO.\n";

$output[] = "\n-- =====================================================================";
$output[] = "-- BAGIAN 4: STRUKTUR LENGKAP 4 VIEW (CREATE OR REPLACE VIEW)";
$output[] = "-- =====================================================================\n";
...
$output[] = "-- =====================================================================";
$output[] = "-- BAGIAN 5: PENGATURAN DEFAULT SISTEM (HANYA JIKA BELUM ADA)";
$output[] = "-- =====================================================================\n";
...
$output[] = "-- =====================================================================";
$output[] = "-- SELESAI: ARSITEKTUR TERBARU & DATA BACKUP LENGKAP TERSINKRONISASI";
$output[] = "-- =====================================================================";

$finalSql = implode("\n", $output);

$targetFile = __DIR__ . '/production_full_migration_with_backup_data.sql';
...
echo "✓ Generated Master Migration File: $targetFile\n";
echo "✓ File Size: " . round(filesize($targetFile) / 1024, 2) . " KB\n";

try {
    echo "Running validation statement-by-statement against database...\n";
    ...
    foreach ($statements as $stmt) {
        ...
        try {
            ...
            echo "[ERROR on stmt #" . ($executed + 1) . "] " . $e->getMessage() . "\n";
            echo "Failed SQL snippet: " . substr($stmt, 0, 200) . "...\n";
            $hasError = true;
            break;
        }
    }
    if (!isset($hasError)) {
        echo "✓ VERIFIED: All $executed SQL statements executed cleanly with ZERO errors!\n";
    }
} catch (Exception $e) {
