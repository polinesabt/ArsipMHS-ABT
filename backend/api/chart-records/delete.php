<?php
/**
 * Soft-delete a chart record and write audit log.
 * DELETE or POST with body: {"section": "...", "record_id": "..."}
 * Auth: admin only
 */
...
try {
    ...
    if (!$row) {
        ...
        echo json_encode(['success' => false, 'error' => 'Record tidak ditemukan atau sudah dihapus.']);
        exit;
    }
    ...
    try {
        $update = $pdo->prepare("UPDATE {$table} SET deleted_at = NOW(), updated_at = NOW() WHERE id = ?");
        $update->execute([$recordId]);

        $logId = bin2hex(random_bytes(18));
        $insLog = $pdo->prepare("INSERT INTO record_change_logs (id, menu_section, record_id, action, admin_id, changed_at, old_data, new_data) VALUES (?, ?, ?, 'deleted', ?, NOW(), ?, ?)");
        $insLog->execute([
            $logId,
            $section,
            $recordId,
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Record telah dihapus (soft delete).',
    ]);
} catch (Exception $e) {
