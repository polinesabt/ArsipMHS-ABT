...
try {
    ...
    if ($evaluationId) {
        $query .= ' LEFT JOIN evaluation_invitations ei ON s.id COLLATE utf8mb4_unicode_ci = ei.student_id AND ei.evaluation_id = ? ';
        $params[] = $evaluationId;
    } else {
        $query .= ' LEFT JOIN evaluation_invitations ei ON 1 = 0 ';
    }

    $conditions = ['s.status = ?', 't.career_status = ?', 'u.role = ?', 'u.is_active = 1'];
    $params[] = 'alumni';
    $params[] = 'working';
    $params[] = 'student';
