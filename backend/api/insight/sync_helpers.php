...
function insight_sync_has_column(PDO $pdo, string $table, string $column): bool {
    static $cache = [];
...
function buildAchievementPayloadForSection(string $section, array $achievement, int $year): array {
    ...
    if ($section === 'student_achievements') {
        $linkProduk = trim((string)($achievement['link_produk'] ?? ''));
        $urlPublikasi = trim((string)($achievement['url_publikasi'] ?? ($achievement['url'] ?? '')));
        $genericLink = trim((string)($achievement['link'] ?? ''));
        $resolvedLink = $linkProduk !== ''
            ? $linkProduk
            : ($urlPublikasi !== '' ? $urlPublikasi : ($genericLink !== '' ? $genericLink : null));
        ...
        return [
            ...
            'url_publikasi' => $urlPublikasi !== '' ? $urlPublikasi : null,
    ...
    if ($section === 'publications') {
        ...
        $seminarContext = $disseminationType === 'seminar'
            ...
            : null;
        $urlPublikasiRaw = trim((string)($achievement['url_publikasi'] ?? ($achievement['url'] ?? '')));
        $urlPublikasi = $urlPublikasiRaw !== '' ? $urlPublikasiRaw : null;
        $resolvedUrl = $seminarContext['url_publikasi'] ?? $urlPublikasi;
        return [
            'category' => $category,
    ...
    if ($section === 'student_products') {
        ...
        $linkProduk = trim((string)($achievement['link_produk'] ?? ''));
        if ($linkProduk === '') {
            $linkProduk = trim((string)($achievement['link'] ?? ''));
        }
        return [
            ...
            'link_produk' => $linkProduk !== '' ? $linkProduk : null,
            'link' => $linkProduk !== '' ? $linkProduk : null,
            'year' => $year,
        ];
    }
    if ($section === 'research_outputs') {
        ...
        if ($jenisPerolehan === null) {
            $jenisPerolehan = $namaDosen !== '' ? 'kolaborasi_dosen' : 'mandiri';
        }
        $urlPublikasi = trim((string)($achievement['url_publikasi'] ?? ($achievement['url'] ?? '')));
        return [
            'category' => $achievement['category'] ?? '',
            ...
            'url_publikasi' => $urlPublikasi !== '' ? $urlPublikasi : null,
            'link' => $urlPublikasi !== '' ? $urlPublikasi : null,
            'year' => $year,
        ];
    }
    return [];
...
function getAchievementWithStudentSnapshot(PDO $pdo, string $achievementId): ?array {
    try {
        $pkiJenisPerolehanExpr = insight_sync_has_column($pdo, 'prestasi_kekayaan_intelektual', 'jenis_perolehan')
            ? 'pki.jenis_perolehan'
            : 'NULL';
        $pkiNamaDosenExpr = insight_sync_has_column($pdo, 'prestasi_kekayaan_intelektual', 'nama_dosen')
            ? 'pki.nama_dosen'
            : 'NULL';
        $pkiUrlPublikasiExpr = insight_sync_has_column($pdo, 'prestasi_kekayaan_intelektual', 'url_publikasi')
            ? 'pki.url_publikasi'
            : 'NULL';
        $stmt = $pdo->prepare("
            SELECT
                ...
                a.penyelenggara,
                a.peringkat,
                COALESCE(ps.jenis_perolehan, {$pkiJenisPerolehanExpr}) AS jenis_perolehan,
                COALESCE(ps.nama_dosen, pp.peran_penulis, {$pkiNamaDosenExpr}) AS nama_dosen,
                COALESCE(ps.penulis, pp.penulis) AS penulis,
                ps.judul_publikasi,
                ps.level_seminar,
                ps.tanggal_publikasi,
                ps.nama_seminar_konferensi,
                COALESCE(ps.url_publikasi, pp.url, {$pkiUrlPublikasiExpr}) AS url_publikasi,
                ppm.link_produk,
                pp.peran_penulis,
...
function syncPublications(PDO $pdo): int {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $seminarContext = $disseminationType === 'seminar'
            ...
            : null;
        $publicationUrl = $seminarContext['url_publikasi'] ?? ($row['url_publikasi'] ?? null);
        if (is_string($publicationUrl)) {
...
function syncStudentProducts(PDO $pdo): int {
    $linkProdukExpr = insight_sync_has_column($pdo, 'prestasi_produk_mahasiswa', 'link_produk')
        ? 'pp.link_produk'
        : 'NULL AS link_produk';
    $stmt = $pdo->query("
        SELECT a.id, a.student_id, a.category, a.subcategory, a.achievement_type, a.tanggal, a.title, {$linkProdukExpr}, s.nim, s.nama, s.prodi, s.jurusan
        FROM achievements a
        LEFT JOIN prestasi_produk_mahasiswa pp ON pp.id_produk_mahasiswa = a.id
        JOIN students s ON s.id = a.student_id AND s.deleted_at IS NULL
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $linkProduk = trim((string)($row['link_produk'] ?? ''));
        $payload = [
...
function syncResearchOutputs(PDO $pdo): int {
    ...
    $researchJenisPerolehanExpr = insight_sync_has_column($pdo, 'prestasi_kekayaan_intelektual', 'jenis_perolehan')
        ? 'pki.jenis_perolehan'
        : 'NULL AS jenis_perolehan';
    $researchNamaDosenExpr = insight_sync_has_column($pdo, 'prestasi_kekayaan_intelektual', 'nama_dosen')
        ? 'pki.nama_dosen'
        : 'NULL AS nama_dosen';
    $researchUrlPublikasiExpr = insight_sync_has_column($pdo, 'prestasi_kekayaan_intelektual', 'url_publikasi')
        ? 'pki.url_publikasi'
        : 'NULL AS url_publikasi';
    ...
    $stmt = $pdo->query("
        SELECT
            ...
            a.title,
            a.description,
            {$researchJenisPerolehanExpr},
            {$researchNamaDosenExpr},
            {$researchUrlPublikasiExpr},
            s.nim,
            s.nama,
            s.prodi,
            s.jurusan
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $tahunPelaporan = (int)date('Y', strtotime($row['tanggal']));
        $payload = buildAchievementPayloadForSection('research_outputs', $row, $tahunPelaporan);
...
function syncStudentAchievements(PDO $pdo): int {
    $publikasiUrlExpr = insight_sync_has_column($pdo, 'prestasi_publikasi', 'url')
        ? 'pp.url'
        : 'NULL';
    $researchOutputUrlExpr = insight_sync_has_column($pdo, 'prestasi_kekayaan_intelektual', 'url_publikasi')
        ? 'pki.url_publikasi'
        : 'NULL';
    $linkProdukExpr = insight_sync_has_column($pdo, 'prestasi_produk_mahasiswa', 'link_produk')
        ? 'ppm.link_produk'
    ...
    $stmt = $pdo->query("
        SELECT
            ...
            a.tingkat,
            a.title,
            COALESCE({$publikasiUrlExpr}, {$researchOutputUrlExpr}) AS url_publikasi,
            {$linkProdukExpr},
            s.nim,
            s.nama,
            s.prodi,
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $tahunPelaporan = (int)date('Y', strtotime($row['tanggal']));
        $payload = buildAchievementPayloadForSection('student_achievements', $row, $tahunPelaporan);
