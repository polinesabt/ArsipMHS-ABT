...
function achievement_store_insert(PDO $pdo, array $config, string $id, array $commonData, array $specificData): void {
    $columns = array_merge([$config['id_col']], achievement_store_common_columns(), $config['specific_columns']);
    ...
    $values = [$id];
    foreach (achievement_store_common_columns() as $column) {
        $values[] = $commonData[$column] ?? null;
    }
    foreach ($config['specific_columns'] as $column) {
...
function achievement_store_update(PDO $pdo, array $config, string $id, array $commonData, array $specificData): void {
    $setColumns = array_merge(achievement_store_common_columns(), $config['specific_columns']);
